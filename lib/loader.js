...
CommandLoader.prototype.run = function run(argv) {
  ...
  if (process.env.ADM_ALIAS) {
    process.argv[1] = process.env.ADM_ALIAS;
  }
  ...
  _.forEach(this.commands, function (action, name) {
    ...
    yargs.command(name, description, action.options || {}, function (options) {
      process.env.ADM_COMMAND = name;
